 ...
 */
public class SqlBaseWalker extends antlr.TreeParser  implements SqlTokenTypes
 {
	private static final Log log = LogFactory.getLog(SqlBaseWalker.class);
	...
	private void beforeStatementCompletion(String statementName) {
        log.debug(statementName+" << finishing up [level="+level+", statement="+ this.statementTypeName+"]");
	}

	private void afterStatementCompletion(String statementName) {
        log.debug(statementName+" << end [level="+level+", statement="+ this.statementTypeName+"]");
		level--;
	}
	...
	public final void fromClause(AST _t) throws RecognitionException {
		...
		try {      // for error handling
			...
			pushFromClause(fromClause_AST,f); 
			handleClauseStart( FROM );
			fromElementList(_t);
			_t = _retTree;
			astFactory.addASTChild(currentAST, returnAST);
